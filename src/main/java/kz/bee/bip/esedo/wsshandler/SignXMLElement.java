...
//        }
    ...
    public void signedCuls(SOAPEnvelope envelope, String elementName) throws UnrecoverableKeyException, TransformerException, KeyStoreException, XPathExpressionException, NoSuchAlgorithmException, XMLSecurityException {
        ...
        if (nodeToSign == null) {
            ...
            X509Certificate cer = (X509Certificate) keyStore.getCertificate(keystoreAlias);
            PrivateKey privateKey = (PrivateKey) keyStore.getKey(keystoreAlias, keystorePassword.toCharArray());
            if (cer == null || privateKey == null) {
                throw new KeyStoreException("Not found PrivateKey with alias \"" + keystoreAlias + "\"");
            }
            ...
            if (doc.getFirstChild() != null) {
                ...
                String base64 = Base64.getEncoder().encodeToString(nodeToString(signedNode).getBytes(StandardCharsets.UTF_8));
                NodeList childList = nodeToSign.getChildNodes();
                Element element = null;
                for(int i = 0; i < childList.getLength(); i++) {
                    Node n = childList.item(i);
                    if("DigiSign".equals(n.getNodeName())) {
                        element = (Element)n;
                        element.setTextContent(base64);
                        break;
                    }
                }
                Node importedNode = document.importNode(nodeToSign, true);
